...
installments_summ_prev <- installments_payments %>%
  #head(n = 500000) %>% 
  left_join(application_train %>% select(-TARGET) %>% bind_rows(application_test), by = "SK_ID_CURR") %>% 
  mutate(AMT.diff = AMT_PAYMENT - AMT_INSTALMENT,
         DAYS.diff = DAYS_ENTRY_PAYMENT - DAYS_INSTALMENT) %>%
...
previous_summ_summ <- previous_application %>% 
  left_join(application_train %>% select(-TARGET) %>% bind_rows(application_test) %>%  select(SK_ID_CURR, AMT_INCOME_TOTAL), by = "SK_ID_CURR") %>% 
  group_by(SK_ID_CURR) %>% 
...
```{r externals}
EXT_master <- application_train %>% 
  bind_rows(application_test) %>% 
  select(-TARGET) %>% 
  select(SK_ID_CURR, contains("EXT_SOURCE")) %>% 
...
application_train %>% 
  #bind_rows(application_test) %>% 
  select(-TARGET) %>% 
  select(SK_ID_CURR, contains("EXT_SOURCE")) %>% skim()
...
bureau_summ_recent <- bureau %>%
  ...
  left_join(application_train %>% bind_rows(application_test) %>% select(-TARGET) %>%  select(SK_ID_CURR, AMT_INCOME_TOTAL, DAYS_EMPLOYED), by = "SK_ID_CURR") %>% 
...
bureau_summ_summ <- bureau %>% 
  left_join(application_train %>% bind_rows(application_test) %>% select(-TARGET) %>% select(-AMT_ANNUITY), by = "SK_ID_CURR") %>% 
  mutate(AMT_CREDIT_MAX_OVERDUE.ratio.inc = AMT_CREDIT_MAX_OVERDUE/(AMT_INCOME_TOTAL+1),
         AMT_CREDIT_MAX_OVERDUE.ratio.annunity = AMT_CREDIT_MAX_OVERDUE/(AMT_ANNUITY+1),
         DAYS_ENDDATE_FACT.ratio.emp = DAYS_ENDDATE_FACT/(DAYS_EMPLOYED+1),
         AMT_CREDIT_SUM_DEBT.ratio.inc = AMT_CREDIT_SUM_DEBT/(AMT_INCOME_TOTAL+1),
...
bureau_summ_master <- bureau_summ_master %>% 
  ...
  left_join(application_train %>% bind_rows(application_test) %>% select(-TARGET) %>% select(DAYS_EMPLOYED, SK_ID_CURR), by = "SK_ID_CURR") %>% 
...
full_dataset <- application_train %>% 
  bind_rows(application_test) %>% 
  left_join(credit_summ_master, by = "SK_ID_CURR") %>% 
  left_join(cardinal_summ_master, by = "SK_ID_CURR") %>% 
  left_join(installments_summ_master, by = "SK_ID_CURR") %>% 
  left_join(previous_summ_master, by = "SK_ID_CURR") %>% 
  left_join(EXT_master, by = "SK_ID_CURR") %>% 
  left_join(bureau_summ_master, by = "SK_ID_CURR")

glimpse(full_dataset)

full_dataset[is.na(full_dataset)] <- -10000000

train <- list(sparse.model.matrix(~., data = full_dataset %>% filter(SK_ID_CURR %in% application_train$SK_ID_CURR) %>%  select(-SK_ID_CURR, -TARGET,  -OCCUPATION_TYPE, -ORGANIZATION_TYPE, -contains("Predicted_TARGET"))), full_dataset %>% filter(SK_ID_CURR %in% application_train$SK_ID_CURR) %>%  pull(TARGET))

 names(train) <- c("data", "label")
 
 test <- list(sparse.model.matrix( ~ ., data = full_dataset %>% filter(SK_ID_CURR %in% application_test$SK_ID_CURR) %>%  select(-SK_ID_CURR, -TARGET,  -OCCUPATION_TYPE, -ORGANIZATION_TYPE, -contains("Predicted_TARGET"))))
 names(test) <- c("data")

dtrain <- xgb.DMatrix(data = train$data, label=train$label)

mod_xgb_tree_final <- xgb.train(data = dtrain,  booster = "gbtree", eta = .1, nrounds = 20)

prediction <- predict(mod_xgb_tree_final, test$data) %>% tibble()
colnames(prediction) <- c("Predicted_TARGET_final")
...
### cooercing missing variables.

full_dataset$NAME_CONTRACT_TYPE.app %>% unique()

full_dataset_fixed <- full_dataset %>% 
  mutate(CREDIT_TYPE.early = case_when(CREDIT_TYPE.early %in% c("Loan for the purchase of equipment") ~ "Too small a type",
                                       !CREDIT_TYPE.early %in% c("Loan for the purchase of equipment") ~ CREDIT_TYPE.early),
         CREDIT_TYPE.small = case_when(CREDIT_TYPE.small %in% c("Loan for the purchase of equipment") ~ "Too small a type",
                                       !CREDIT_TYPE.small %in% c("Loan for the purchase of equipment") ~ CREDIT_TYPE.small),
         CREDIT_TYPE.recent = case_when(CREDIT_TYPE.recent %in% c("Loan for the purchase of equipment") ~ "Too small a type",
                                       !CREDIT_TYPE.recent %in% c("Loan for the purchase of equipment") ~ CREDIT_TYPE.recent),
         CODE_GENDER = case_when(CODE_GENDER %in% c("XNA") ~ "M",
                                       !CODE_GENDER %in% c("XNA") ~ CODE_GENDER),
         NAME_INCOME_TYPE = case_when(NAME_INCOME_TYPE %in% c("Maternity leave") ~ "Working",
                                      !NAME_INCOME_TYPE %in% c("Maternity leave") ~ NAME_INCOME_TYPE),
         NAME_FAMILY_STATUS = case_when(NAME_FAMILY_STATUS %in% c("Unknown") ~ "Single / not married",
                                        !NAME_FAMILY_STATUS %in% c("Unknown") ~ NAME_FAMILY_STATUS),
         NAME_CONTRACT_TYPE.dec = case_when(NAME_CONTRACT_TYPE.dec %in% c("XNA") ~ "-1e+07",
                                            !NAME_CONTRACT_TYPE.dec %in% c("XNA") ~ NAME_CONTRACT_TYPE.dec),
         CODE_REJECT_REASON.dec = case_when(CODE_REJECT_REASON.dec %in% c("SYSTEM") ~ "XNA",
                                            !CODE_REJECT_REASON.dec %in% c("SYSTEM") ~ CODE_REJECT_REASON.dec),
         jobrisk = case_when(jobrisk %in% c("HIGH") ~ "-1e+07",
                             !jobrisk %in% c("HIGH") ~ jobrisk),
         NAME_CONTRACT_TYPE.app = case_when(NAME_CONTRACT_TYPE.app == "XNA" ~ "-1e+07",
                                            !NAME_CONTRACT_TYPE.app == "XNA" ~ NAME_CONTRACT_TYPE.app))
         
## Having another go   
        
train <- list(sparse.model.matrix(~., data = full_dataset_fixed %>% filter(SK_ID_CURR %in% application_train$SK_ID_CURR) %>%  select(-SK_ID_CURR, -TARGET,  -OCCUPATION_TYPE, -ORGANIZATION_TYPE, -contains("Predicted_TARGET"))), full_dataset_fixed %>% filter(SK_ID_CURR %in% application_train$SK_ID_CURR) %>%  pull(TARGET))

names(train) <- c("data", "label")
 
test <- list(sparse.model.matrix( ~ ., data = full_dataset_fixed %>% filter(SK_ID_CURR %in% application_test$SK_ID_CURR) %>%  select(-SK_ID_CURR, -TARGET,  -OCCUPATION_TYPE, -ORGANIZATION_TYPE, -contains("Predicted_TARGET"))))
names(test) <- c("data")

dtrain <- xgb.DMatrix(data = train$data, label=train$label)

mod_xgb_tree_final <- xgb.train(data = dtrain,  booster = "gbtree", eta = .02, nrounds = 2000)

prediction <- predict(mod_xgb_tree_final, test$data) %>% tibble()
colnames(prediction) <- c("TARGET") 

prediction<- prediction %>% bind_cols(application_test %>% select(SK_ID_CURR))
prediction <- prediction %>% select(SK_ID_CURR, TARGET)
prediction <- prediction %>% 
  mutate(TARGET = rescale(TARGET))
         
prediction %>% write_csv("kaggleprediction3.csv")

prediction %>% skim()

imp <- xgb.importance(model = mod_xgb_tree_final)
imp %>% 
  filter((Gain) > .0035, (Cover > .0035)) %>% 
  ggplot(aes(x = log(Gain), y = log(Cover), col = (Frequency)))+
  geom_point()+
