...
installments_summ_master <- installments_summ_last %>% 
  inner_join(installments_summ_length, by = "SK_ID_CURR") %>% 
  inner_join(installments_summ_payment, by = "SK_ID_CURR") %>% 
  inner_join(installments_summ_short, by = "SK_ID_CURR") %>% 
  inner_join(loan_count, by = "SK_ID_CURR") 

installments_summ_prev %>%
  mutate(SK_ID_CURR = as.factor(SK_ID_CURR),
         SK_ID_PREV = as.factor(SK_ID_PREV)) %>% 
  head(n = 50000) %>% 
  skim()
```
