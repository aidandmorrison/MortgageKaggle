...
application_train_test <- application_train_test %>% select(-sd.bal.med)
 train <- list(sparse.model.matrix(~., data = application_train_train %>% select(-TARGET, -OWN_CAR_AGE)), application_train_train$TARGET)
 names(train) <- c("data", "label")
 test <- list(sparse.model.matrix( ~ ., data = application_train_test %>% select(-TARGET, - OWN_CAR_AGE)), application_train_test$TARGET)
...
## Here's the mistake coming up, why are the lengths different?
#  dtrain <- xgb.DMatrix(data = train$data, label=train$label)
#  dtest <- xgb.DMatrix(data = test$data, label=test$label)
#  watchlist <- list(train=dtrain, test=dtest)
# #
#  model <- xgb.train(data = dtrain, max.depth = 20, booster = "gblinear", eta = .2, nrounds = 15, watchlist = watchlist)
#
